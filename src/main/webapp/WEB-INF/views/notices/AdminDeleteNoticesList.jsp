...
<form:form  action="ViewDeactivatedNotices" id="viewDform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
  <input type="hidden" id="did" name="id" />  
  </form:form>
  
  <form:form  action="adminDeleteViewNotices" id="viewPform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
...
function selectStatus(){
	...
	var tendorStatus = $("#tendorStatus").val();//alert(tendorStatus)
		...
		var issueHTML = '<div class="box"><div class="box"><div class="box-header"><h3 class="box-title">Notifications Data Table</h3></div>'+
		...
		'<th>Type</th><th id="tableAction">Action</th></tr>'+
		'</thead>'+
		'<tbody>';	
		
		
		$.ajax({
		    url : 'getNoticesByDeleteStatus?tendorStatus='+tendorStatus,      
		    	...
		    	//alert(response.length)
		    	 for(var i=0;i<response.length;i++){
		    		if(response[i].delete_approval_status=='Deactivate'){
						...
						'<td><input type="button" value="View" class="btn btn-primary btn-flat margin" onclick="return viewD('+response[i].noticeID+')"/></td>';
		    		...
		    		if(response[i].delete_approval_status==='Pending')
					{
						...
						'<td>'+response[i].title_h+'</td>'+	
						'<td>'+response[i].type+'</td>'+			
						'<td><input type="button" value="Change Status" class="btn btn-primary btn-flat margin" onclick="return viewP('+response[i].noticeID+')"/></td>';
