...
<form:form  action="viewDeactivatedDepartment" id="viewDform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
  <input type="hidden" id="did" name="id" />  
  </form:form>
  <form:form  action="adminDeleteViewDepartment" id="viewPform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
...
function selectStatus(){
	...
	var tendorStatus = $("#tendorStatus").val();//alert(tendorStatus)
		...
		'<thead><tr><th>S.N.</th><th>Name in English</th><th>Name in Marathi</th>'+
		...
		$.ajax({
		    url : 'getDepartmentByDeleteStatus?tendorStatus='+tendorStatus,      
		    	...
		    	//alert(response.length)
		    	 for(var i=0;i<response.length;i++){
		    		if(response[i].delete_approval_status=='Deactivate'){
						...
						'<td>'+response[i].departmentName_h+'</td>'+
						'<td><input type="button" value="View" class="btn btn-primary btn-flat margin" onclick="return viewD('+response[i].deptId+')"/></td>';
						/* '<td><a href="viewDeactivatedDepartment?id='+response[i].deptId+'">View</a></td>';  */
		    		...
		    		if(response[i].delete_approval_status==='Pending')
					{
						...
						'<td>'+response[i].departmentName+'</td>'+
						'<td>'+response[i].departmentName_h+'</td>'+
						'<td><input type="button" value="Change Status" class="btn btn-primary btn-flat margin" onclick="return viewP('+response[i].deptId+')"/></td>';
						/* '<td><a href="adminDeleteViewDepartment?id='+response[i].deptId+'">Change Status</a></td>';  */
					}
