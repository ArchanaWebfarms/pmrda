...
<html prefix="og: http://ogp.me/ns#" lang="en-gb" dir="ltr" class='com_content view-featured itemid-550 j38 mm-hover'>
  ...
  <meta name="viewport" content="width=device-width" />
     <meta name="description" content=" Central goverments rules for pmrda"/>
      <title>.:: <spring:message code="CG.pagetitle"  />  ::.</title>
      ...
      <%-- <div class="container">
         <section class="wrap t3-navhelper" style="margin-top: 0px;">
               <div class="row">
                  ...
                  <li><i class="fas fa-caret-right"></i> <span><spring:message code="CG.pagetitle"  /></span></li>
                  </ol>
               </div>
            </section>
            <hr/>
      ...
      <div id="t3-mainbody  maincontent"    class="container-fluid t3-mainbody dl-container" style="margin-top:30px;">
         <div class="row">
            <div class="itemHeader">
               ...
               <spring:message code="CG.t3-mainbody.itemTitle.h2"  /> <a href ="Archive_centralGR" style="color:green;font-size: 22px"  class="archiveLink"><spring:message code="archive.archived.notice" /></a>
...
/* function SubmitDatfes(){
  ...
  if(language=="marathi")
	...
	'<tbody>';	
     ...
     $.ajax({
		 url:"sendbothdates?fromdate="+fromdate+"&todate="+todate+"&Type=CENTRAL GOV GR",
					...
					for(var j=0;j<response.attachment.length;j++){   
						if(response.notice[i].noticeID==response.attachment[j].moduleID){
							if(language=="marathi" && response.attachment[j].module_type=="Marathi Attachment"){
									issueHTML+='<a href="attachmentDownload?path='+response.attachment[j].path+'" >'+response.attachment[j].title+'</a><br> ';  
...
function searchNoticesByDepartment(){
  ...
  var departments = $("#departments").val();
     ...
     $.ajax({
			url:"searchNoticesByDepartment?departments="+departments+"&noticeType=CENTRAL GOV GR"+"&category=NOTICE",
			success:function(response){
				//alert(response.length);
...
function SubmitDates(){
  ...
  if(language=="marathi")
     ...
     $.ajax({
		 url:"noticebydates?fromdate="+fromdate+"&todate="+todate+"&type=CENTRAL GOV GR",
			success:function(response){
				console.log(response)
