...
<form:form  action="viewDeactivatedTender" id="viewDform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
  <input type="hidden" id="did" name="id" />  
  </form:form>
  
<form:form  action="adminDeleteViewTender" id="viewPform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
...
function selectStatus(){
	...
	var tendorStatus = $("#tendorStatus").val();//alert(tendorStatus)
		...
		var issueHTML = '<div class="box"><div class="box"><div class="box-header"><h3 class="box-title">Tender Notice Data Table</h3></div>'+
		...
		'<th>Publish Date</th><th>Closing Date</th><th>Opening Date</th><th id="tableAction">Action</th></tr>'+
		'</thead>'+
		'<tbody>';	
		
		
		$.ajax({
		    url : 'getTenderByDeleteStatus?tendorStatus='+tendorStatus,      
		    	...
		    	//alert(response.length)
		    	 for(var i=0;i<response.length;i++){
		    		if(response[i].delete_approval_status=='Deactivate'){
						...
						'<td>'+response[i].publish_date+' '+response[i].publish_time+'</td>'+	
						'<td>'+response[i].closingDate+' '+response[i].closing_time+'</td>'+	
						'<td>'+response[i].openingDate+' '+response[i].opening_time+'</td>'+	
						'<td><input type="button" value="View" class="btn btn-primary btn-flat margin" onclick="return viewD('+response[i].id+')"/></td>';
						/* '<td><a href="viewDeactivatedTender?id='+response[i].id+'">View</a></td>';  */
		    		...
		    		if(response[i].delete_approval_status==='Pending')
					{
						...
						/* '<td><a href="adminDeleteViewTender?id='+response[i].id+'">Change Status</a></td>'; */ 
					}
...
$( document ).ready(function() {
	$("#adminTenderDashboard").addClass("active");
