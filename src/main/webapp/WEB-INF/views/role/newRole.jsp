...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>  New Role Registration   </h1>
      <ol class="breadcrumb">
        <li><a href="home"><i class="fa fa-dashboard"></i> <b>Home</b></a></li>
        <li><a href="listOfRoles">Role List</a></li>
      ...
      <div class="box box-default">
        <div class="box-header with-border">
         ...
         <!-- /.box-header -->
           <div class="box-body" >
             <%--  <%String add=null;%>	 --%>
		<form:form action="saveRoleRegistration?${_csrf.parameterName}=${_csrf.token}" method="POST" modelAttribute="roles" commandName="roles" class="form-horizontal">
			
			
			<form:hidden path="role_id" id="role_id" class="form-control"/>
			
			<input type="hidden" id="rolename" value="">
		   <div class="row">
				<div class="form-group col-md-12">
					<label class="col-md-3 control-lable" for="role_type">Enter Role</label>
					<span style="color: red" id="role_typeError"></span>
					<form:errors style="color: red"  path='role_type' />
					<div class="col-md-7">
						<form:input type="text" path="role_type" id="role_type" onkeyup="checkRole();" class="form-control"/>						
					</div>
				</div>
			</div>
			...
			<%-- <div class="row">
	          ...
	          <div class="box-footer">
                  <div class="form-actions floatRight">
					 <input type="submit" value="Save"  class="btn btn-primary"><%--  onclick="return save()" --%>
				  	 <input type="button" value="Cancel" onclick="location.href='listOfRoles'" class="btn btn-success">
...
function checkRole(){
	 ...
	 if(role_type){
		 $.ajax({
				url:"checkRole?role_type="+role_type,
				success:function(response){	
					//alert("response.role_type "+response.role_type);
					$("#rolename").val("");
					if(response.role_type != null){
						//alert(response.role_type);
						$("#rolename").val(response.role_type);
						return false;
					}return true;
				}
			});
		...
		function save(){
			if(validateRole()){
				var value=document.getElementById("rolename").value;
				//alert("value "+value);
				  if(value===""){
				        ...
				        document.getElementById("role_typeError").innerHTML="Role Type should be unique";
			...
			success:function(response){
		     for(var i=0;i<response.length;i++){
					 var addressinput;
					  ...
					  var title=document.getElementById("role_type").value;
					  var description=document.getElementById("description").value;
					 
					   $(".form-control").change(function() {
		              addressinput = $(this).val();
		                });
					   if(new RegExp(response[i]).test(title) || new RegExp(response[i]).test(description) )
					    {
