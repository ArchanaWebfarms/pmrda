 ...
 <div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1> Current Openings Data Table<!-- <small>advanced tables</small> --></h1>
      <ol class="breadcrumb">
        ...
        <li class="active">Admin Openings List</li>
      </ol>
    </section>
...
<form:form  action="ViewDeactivatedOpening" id="viewDform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
  <input type="hidden" id="did" name="id" />  
  </form:form>
  <form:form  action="adminDeleteViewOpening" id="viewPform" oncontextmenu="return false" onkeydown="return false;" onmousedown="return false;" >  	
...
function selectStatus(){
	...
	var tendorStatus = $("#tendorStatus").val();//alert(tendorStatus)
		...
		var issueHTML = '<div class="box"><div class="box"><div class="box-header"><h3 class="box-title">Current Openings Data Table</h3></div>'+
		'<div class="box-body"><table id="example1" class="table table-bordered table-striped">'+ 
		'<thead><tr><th>S.N.</th><th>Post in English</th><th>Post in Marathi</th>'+
		'<th>Opening Status</th><th id="tableAction">Action</th></tr>'+
		'</thead>'+
		'<tbody>';	
		
		
		$.ajax({
		    url : 'getOpeningByDeleteStatus?tendorStatus='+tendorStatus,      
		    	...
		    	//alert(response.length)
		    	 for(var i=0;i<response.length;i++){
		    		if(response[i].delete_approval_status=='Deactivate'){
						...
						'<td>'+response[i].post_h+'</td>'+	
						'<td>'+response[i].opening_status+'</td>'+		
						'<td><input type="button" value="View" class="btn btn-primary btn-flat margin" onclick="return viewD('+response[i].id+')"/></td>';
						/* '<td><a href="ViewDeactivatedOpening?id='+response[i].id+'">View</a></td>';  */
		    		...
		    		if(response[i].delete_approval_status==='Pending')
					{
						...
						/* '<td><a href="adminDeleteViewOpening?id='+response[i].id+'">Change Status</a></td>';  */
					}
...
$( document ).ready(function() {
	$("#adminOpeningDashboard").addClass("active");
